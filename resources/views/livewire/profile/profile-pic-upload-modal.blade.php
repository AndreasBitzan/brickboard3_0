<x-modal.card title="{{ __('Ändere dein Profilbild') }}" blur wire:model.defer="cardModal">
    @if (
        $profilePic &&
            ($profilePic->extension() == 'png' ||
                $profilePic->extension() == 'jpg' ||
                $profilePic->extension() == 'jpeg' ||
                $profilePic->extension() == 'gif'))
        <div class="flex justify-center items-center">
            <img class="aspect-square object-cover h-40" src="{{ $profilePic->temporaryUrl() }}">
        </div>
    @endif
            ...
            <div wire:loading.remove class="text-center">
                ...
                <div class="mt-4 flex text-sm text-center leading-6 text-gray-600">
                    ...
                    <label for="file-upload"
                        ...
                        <span class="px-4">{{ __('Lade ein Bild hoch') }}</span>
                        <input id="file-upload" name="file-upload" wire:model="profilePic" type="file"
                            class="sr-only">
                    </label>
                </div>
                <p class="text-xs leading-5 text-gray-600 dark:text-white">{{ __('JPG, PNG, GIF bis zu 5MB') }}</p>
                <p class="text-xs leading-5 text-gray-600 dark:text-white">{{ __('Am besten Seitenverhältnis 1:1') }}
                </p>
            </div>
        </div>
    </div>
